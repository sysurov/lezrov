...
void TIM2_IRQHandler(void)
{
	...
	if(TIM_GetITStatus(TIM2,TIM_IT_Update)!=RESET)
	{
		...
		{
			if(CPGMode==0){
				CalculateSitaLevel(1,10,10,Reg.Speed,Reg.Direct);
				if(Reg.Speed==0)
				{
                    ResetData();
					for(i=0;i<JointCount;i++)
                    {
                        Sita[i]=KSita*XTem[i][Reg.Direct];
                    }
				}
			}else if(CPGMode==1){
		...
		TIM2->CCR1=3000+Sita[2]+Reg.ServoOffset[0];
		TIM2->CCR2=3000+Sita[2]+Reg.ServoOffset[0];
		TIM2->CCR3=3000+Sita[2]+Reg.ServoOffset[0];
		TIM2->CCR4=3000+Sita[2]+Reg.ServoOffset[0];
	}
}
